...
항만 운영사, 물류기업, 창고 운영자는 선박 스케줄링, 야드 용량 배분, 인력 운용을 위해 정확한 TEU(20피트 컨테이너 환산 단위) 수요예측이 필요. 업계 관행은 규칙 기반 계절 조정에 의존하고 있어, COVID-19 봉쇄, 파업, 유가 급변 등 충격 상황에서 예측 정확도가 급락하는 한계가 존재함.
...
원본 데이터셋에는 TEU 수치만 포함되어 있고 외부변수는 없음. 공개 API에서 월별 5개 공변량을 직접 수집/가공.
...
**설계 근거**: 트리 기반 그래디언트 부스팅은 대규모 데이터 없이도 테이블형 피처를 효율적으로 처리하여 현업 수요예측에 광범위하게 활용.
...
**Direct Multi-Step 전략**: 각 예측 구간 h ∈ {1, 3, 6, 12}에 대해 독립적인 LGBMRegressor를 학습. 재귀 예측(recursive forecasting)의 오차 누적 문제를 회피하기 위한 설계.
...
```
`num_leaves=15`, `min_child_samples=5`는 93행 소규모 데이터에서의 과적합 방지를 위해 의도적으로 보수적으로 설정.
...
실제 운영 환경을 모사한 설계. 매월 새 데이터가 쌓이면 모델을 재학습하는 방식, 미래 정보 유출이 없음. SARIMAX는 더 긴 시계열 분할을 사용하므로 n=57~68.
